...
theorem histogramFold_get (xs : List ℕ) (counts : Array ℕ) (key : ℕ)
  ...
  | cons x xs ih =>
      ...
      by_cases heq : x = key
      · subst x
        simp [Array.getElem?_modify, Array.getElem?_eq_getElem hkey,
          Nat.add_assoc, Nat.add_comm]
      · simp [Array.getElem?_modify, heq,
          Array.getElem?_eq_getElem hkey]
...
theorem count_expand_range (counts : ℕ → ℕ) (n key : ℕ) :
  ...
  | succ n ih =>
      ...
      by_cases heq : n = key
      · subst key; simp
      · have hbeq : (n == key) = false := by simp [heq]
        rw [hbeq]
        by_cases hlt : key < n
        · simp [hlt, Nat.lt_succ_of_lt hlt]
        · have : ¬ key < n + 1 := by omega
          simp [hlt, this]

theorem countingSortResult_count (xs : List ℕ) (key : ℕ) :
    (countingSortResult xs).count key = xs.count key := by
  rw [countingSortResult_eq, count_expand_range]
  by_cases hkey : key < keyRange xs
  · rw [ite_eq_left hkey, histogram_get]
  · have hnot : key ∉ xs := fun hx => hkey (mem_lt_keyRange xs key hx)
    simp [hkey, List.count_eq_zero.mpr hnot]
