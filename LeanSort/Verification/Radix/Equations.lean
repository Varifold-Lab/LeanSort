...
theorem digitPass_eq_filters (bit : ℕ) (xs : List ℕ) :
  ...
  simp [digitPass, partitionTrace_parts, Function.comp_def]
...
theorem digitPass_eq_partition (bit : ℕ) (xs : List ℕ) :
    digitPass bit xs =
      (xs.partition (fun x => decide (x / 2 ^ bit % 2 = 0))).1 ++
      (xs.partition (fun x => decide (x / 2 ^ bit % 2 = 0))).2 := by
  rw [digitPass, partitionTrace_parts]
...
theorem radixSortResult_eq_passes (xs : List ℕ) :
    radixSortResult xs = (List.range (radixBits xs)).foldl
      (fun result bit => digitPass bit result) xs := passesTrace_result _ _
